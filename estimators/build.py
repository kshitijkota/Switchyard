...
direct and dr share ONE Q-model fit on the legacy logs; switchyard fits its own on the
...
def build_all(legacy_ds: LoggedDataset, explore_ds: LoggedDataset) -> dict:
    """Return {name -> fitted Method} for direct, ips, snips, dr, switchyard."""
    qmodel = QModel().fit(legacy_ds)
    combined = LoggedDataset.concat(legacy_ds, explore_ds)
    # switchyard shares the SAME (legacy) Q-model as direct/dr, so the only thing that
    # separates switchyard from dr is the exploration data in its DR correction term.
    return {
        ...
        "switchyard": switchyard.build(combined, qmodel=qmodel),
    }
