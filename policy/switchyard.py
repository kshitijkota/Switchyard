"""`switchyard` — the method this project argues for (AGENT_BRIEF §6, item 4).
...
switchyard fits its own Q-model on the *combined* data (legacy + exploration), so the
...
def build(combined: LoggedDataset, qmodel: QModel | None = None) -> Method:
    ...
    return Method("switchyard", "dr", weight_clip=WEIGHT_CLIP).fit(combined, qmodel=qmodel)
